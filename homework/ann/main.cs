...
public static class main{
	...
	public static void Main(){
		...
		WriteLine("Making interpolation with 20 neurons...");
		myAnn = new ann(20);
		myAnn.train(xs,ys);
		//create interpolation set
		toWrite = "";
		...
		File.WriteAllText("txts/20neuronInterp.txt",toWrite);
		...
		for(int i = 0; i < interpolationPoints; i++){
			double x = -1 + 2.0 * i / interpolationPoints;
			toWrite += $"{x}\t{myAnn.antiderivative(x)}\t{myAnn.derivative(x)}\t{myAnn.derivative2(x)}\n";
		}
		...
		for(int i = 0; i < interpolationPoints; i++){
			...
			toWrite += $"{x}\t{myAnn.antiderivative(x)}\t{myAnn.derivative(x)}\t{myAnn.derivative2(x)}\t{myAnn.response(x)}\n";
		}
