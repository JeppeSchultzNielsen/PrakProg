...
public static class main{
	...
	public static void Main(){
		...
		a[0] = 0;
		a[1] = 0;
		(res, err) = mcint.strataint(x => x[0]*x[0]*Sin(x[1]), a, b, 1000,100);
		WriteLine($"Int( r^2sin(theta) ) with r from 0 to 1 and theta from 0 to 2pi is calculated as: {res} +- {err}" );
		WriteLine($"It should be 1/(2pi) * 4/3 * pi * 1^3 = 2/3 = {2.0/3.0}");
		(res, err) = mcint.strataint(x => x[0]*x[0]*Sin(x[1]), a, b, 10000,100);
		WriteLine($"Int( r^2sin(theta) ) with r from 0 to 1 and theta from 0 to 2pi is calculated as: {res} +- {err}" );
		WriteLine($"It should be 1/(2pi) * 4/3 * pi * 1^3 = 2/3 = {2.0/3.0}");
		(res, err) = mcint.strataint(x => x[0]*x[0]*Sin(x[1]), a, b, 100000,100);
		WriteLine($"Int( r^2sin(theta) ) with r from 0 to 1 and theta from 0 to 2pi is calculated as: {res} +- {err}" );
		WriteLine($"It should be 1/(2pi) * 4/3 * pi * 1^3 = 2/3 = {2.0/3.0}");
		b[0] = 1;
		b[1] = 1;
		a[0] = 0;
		a[1] = 0;
		(res, err) = mcint.strataint(x => (Exp( - (x[0]*x[0] + x[1]*x[1])) ), a, b, 1000,100);
		WriteLine($"Int( e^(x^2+y^2) ) with x from 0 to 1 and y from 0 to 1 is calculated as: {res} +- {err}");
		WriteLine($"It should be 0.557746");
		WriteLine($"");
		WriteLine("Calculating some test integrals with stratified Monte Carlo integrator with 1000 points: ");*/
