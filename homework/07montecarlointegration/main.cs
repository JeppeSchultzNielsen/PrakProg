...
public static class main{
	...
	public static void Main(){
		...
		WriteLine("-------");
		WriteLine("Part c)");
		WriteLine("Now I will calculate the integrals with stratified sampling.");
		b[0] = 1;
		b[1] = 1;
		a[0] = 0;
		a[1] = 0;
		string path = "txts/test.txt";
		using (StreamWriter sw = File.CreateText(path)){}
		(res, err) = mcint.strataint(circle, a, b, 10000,200,path);
		WriteLine("An example of the distribution of points when using stratified sampling can be seen on StratifiedSampling.svg.");
		b[0] = 1;
		b[1] = 1;
		a[0] = 0;
		a[1] = 0;
		...
		WriteLine($"A comparison of the variance as a function of number of points between all the sampling methods has been saved to Convergence.svg.");
		string toWrite = "";
		for(int i = 0; i < 3; i++){
			for(int j = 0; j < 5; j++){
		...
		File.WriteAllText("txts/convergence.txt", toWrite);
	}
}
